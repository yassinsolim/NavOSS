...
A connected display also holds a `CLBackgroundActivitySession`. Under When in Use authorization that session is what keeps location flowing after the phone's screen sleeps, and without it the CarPlay map froze until the phone was physically woken, since the driver's phone is normally locked in a pocket or cradle. Idle phone use deliberately holds no session: the map is not visible then, so it would cost battery and show the background indicator for nothing.
