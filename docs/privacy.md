...
In a build where Google Places ratings are explicitly enabled, opening a point-of-interest sheet also sends that selected public name and coordinate directly from the phone to Google Places. Google's Places UI Kit chooses the place and renders the star rating, rating count, and required Google attribution inside its own component. NavOSS does not receive those values as application data, combine them with OpenStreetMap data, cache them, or send them to the NavOSS server. Builds without the restricted Google key do not make this request and show that the Google rating is unavailable.
...
Call and website actions open the phone dialer or an external browser/app only after the user chooses them. **Read reviews on Google Maps** is a separate external action and sends the selected public place name and coordinate to Google only when chosen. NavOSS does not fetch, display, cache, or scrape review text. In a Google-enabled build, the rating-only request described above can occur when a point-of-interest sheet opens; the external reviews search still occurs only after the user chooses it.
...
The NavOSS origin is in Alberta, but Cloudflare, OpenFreeMap, Apple, Google when ratings are enabled or the external reviews link is chosen, GitHub, and a user-selected share destination may process data outside Canada as described in their policies. No security measure can guarantee absolute protection.
