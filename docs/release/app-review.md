...
- Bundle ID `org.navoss.mobile`, version `0.1.0`, and foreground-only location configuration are present.
...
- `/ready` checks both providers, production search returns `nominatim-self-hosted`, and all 57 official Calgary camera records load publicly.
...
- All five containers use journald with a seven-day/512 MiB limit; SSH and firewall logs rotate within seven days, and report-database backups retain 14 days.
...
| 2.1 App Completeness                                       | High                                          | The production API, search, routing, and camera paths are live. A reviewer outside Calgary still cannot create an in-coverage route with current-location-only origin selection.               | Complete the backend soak and add a normal manual-origin flow or documented built-in Calgary route preview before Beta App Review. Test the exact store build without Metro or the development Mac.                                                                  |
...
| 2.3 Accurate Metadata                                      | High                                          | The draft avoids traffic and parity claims, but screenshots and final descriptions do not yet exist.                                                                                           | Capture the submitted production build in use. Show only implemented features. State Calgary coverage and no live traffic. Do not claim Waze, Google Maps, or Apple Maps parity, exhaustive search, background guidance, CarPlay, lane guidance, or offline routing. |
| 2.3.1 Review Notes                                         | High                                          | Non-obvious coverage, camera direction filtering, and foreground-only behavior need specific instructions.                                                                                     | Provide exact taps, a deterministic test place, coverage limits, account-free access, backend status, camera provenance, location behavior, and the reviewer route-preview path. Do not hide or omit a demo feature.                                                 |
...
| 2.5.4 Background services                                  | Low now                                       | Background location is disabled and no background guidance is claimed.                                                                                                                         | Confirm the signed archive has no undeclared background location mode. Reopen permissions, privacy, battery, and review scope before adding background navigation.                                                                                                   |
...
| 5.1.1 Permission and minimization                          | Medium                                        | Foreground precise location is core to current navigation and has a complete purpose string. Search/map browsing can work without location, but route calculation cannot.                      | Verify denial and revocation behavior. Request location only when the user invokes location-dependent functionality. Keep manual search usable after denial and avoid unrelated permissions.                                                                         |
...
| 5.1.5 Location Services                                    | High                                          | Location directly supports navigation and is not used for emergency service or autonomous vehicle control. It is used only while the app is open.                                              | Keep App Store privacy answers, policy, purpose string, app behavior, and signed entitlements consistent. Document precise-location transmission and make stopping guidance stop location updates.                                                                   |
...
| -------------------------------------------- | --------------------------------------------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Search                                       | Search text and optional approximate search origin        | Cloudflare transit; operator-hosted API and Nominatim | Sent in a JSON body, processed in memory, and discarded after the response. Tested values are absent from logs and backups; classify as Not Collected under Apple's real-time-service definition unless provider settings change. |
| Initial route                                | Precise origin and destination coordinates                | Cloudflare transit; operator-hosted API and Valhalla  | Processed in memory and discarded after the response; tested coordinates are absent from logs and backups. Classify as Not Collected under the current real-time-service definition unless provider settings change.              |
| Reroute                                      | Latest precise route origin and destination               | Same as routing                                       | Uses the same transient body-only path and retention controls. Reassess before adding request tracing, route history, traffic telemetry, or other persistence.                                                                    |
...
- Calgary Open Data Terms of Use, dataset ID `dv2f-necx`, supplier, update cadence, and the current source snapshot date;
...
1. Add manual origin search so a reviewer can choose a Calgary start and destination without changing device location. This is normal user functionality and is the preferred solution.
2. Add a clearly visible, documented Calgary route-preview mode containing the same production route response and UI. Do not hide it behind an undocumented gesture or review-only server flag.
...
- [ ] Add and test the manual Calgary origin or visible route-preview path.
...
- decide whether foreground-only, screen-on navigation is an acceptable public product or complete background/spoken guidance first; and
