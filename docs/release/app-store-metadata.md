...
The generated app-level privacy manifest currently declares no collected data and no tracking. Keep that only for an artifact whose production facts support it. A Google-enabled artifact embeds additional Google Places declarations and requires matching App Store Connect answers, hosted policy, review notes, and archive inspection.
